...
class TodoViewModel @Inject constructor(private val todoRepository: TodoRepository) : ViewModel() {
    ...
    private fun insertTodo(todo: Todo) {
        viewModelScope.launch(Dispatchers.IO) { todoRepository.insert(todo) }
    }

    private fun deleteTodo(todo: Todo) {
        viewModelScope.launch(Dispatchers.IO) { todoRepository.delete(todo) }
    }

    private fun updateTodo(todo: Todo) {
        viewModelScope.launch(Dispatchers.IO) { todoRepository.update(todo) }
    }
